...
const AdvertiserCreate = ({ onClose, onRefresh }) => {
    ...
    const fetchTypes = async () => {
      try {
        ...
        if (response.ok) {
          ...
          throw new Error("Не удалось загрузить типы рекламодателей");
        }
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${config.api}/admin/advertisers/create/`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setError(data.detail || "Ошибка создания рекламодателя");
      }
    } catch (err) {
      console.error(err);
